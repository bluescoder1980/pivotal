...
[System.Web.Services.WebService(Namespace="http://IrvinePacific/QA/TICPivotalQAService")] 
public class TICPivotalQAServiceSvc : System.Web.Services.WebService
{
    ...
    private string strcfSYS_NAME = ConfigurationSettings.AppSettings["PivotalSystemName"];
...
//    }
    ...
    { 
        ...
        IQAController controller = QAControllerFactory.GetQAController();
        UserObj userObj = controller.GetUserForLogin(userName);

        //Test against hard-coded user login info to authenticate for prototype
        if ( userObj.loginName != null && userObj.loginName.ToUpper()!= userName.ToUpper())
        {
    ...
    public ScheduledInspectionWrapper GetScheduledInspectionsForUser(string userLogin, CompanyType type)
    {
        ...
        //Get Controller from factory
        IQAController controller = QAControllerFactory.GetQAController();
        ScheduledInspectionWrapper wrapper = controller.GetScheduledInspectionsForUser(userLogin, type);
        return wrapper;
    }
    ...
    public InspectionListWrapper GetInspectionsForUser(string userLogin, string status, CompanyType type)
    {
        //Call to controller to get list of inspections for user logged in.
        IQAController controller = QAControllerFactory.GetQAController();
        InspectionListWrapper wrapper = controller.GetInspectionList(userLogin, status, type);
        return wrapper;
    }
    ...
    /// create a new record in Pivotal, a seperate method will need to be called to commit
    ...
    public string CreateNewInspection(string projectId, string phaseName, 
        string inspectionTypeId, string inspectionScope, string createdById, string inspectorId,
        string[] selectedScopeItems)
    {
        //Call controller to get the new inspection template
        IQAPBSController controller = QAPBSControllerFactory.GetQAPBSController(strcfSYS_NAME);
        string newInspectionId = controller.InsertNewInspectionIntoPivotal(projectId, phaseName, inspectionTypeId,
            inspectionScope, createdById, inspectorId, selectedScopeItems);
        return newInspectionId;
    }

    /// <summary>
    /// This method will return an inspection object populated with all the necessary
    /// information for the inspector to complete the inspection
    /// </summary>
    /// <param name="inspectionId"></param>
    /// <returns></returns>
    [WebMethod(Description = "Loads the inspection record with secondary data.")]
    public Inspection LoadInspectionById(string inspectionId, LoadActionsForInspection action, string lastUpdatedByUserId)
    {
        IQAController controller = QAControllerFactory.GetQAController(strcfSYS_NAME);
        Inspection inspObj = controller.LoadExistingInspection(inspectionId, action, lastUpdatedByUserId);
        return inspObj;
    }
    ...
    public Inspection UpdateInspection(Inspection inspection, ActionForInspection action)
    {
        IQAPBSController controller = QAPBSControllerFactory.GetQAPBSController(strcfSYS_NAME);
        Inspection inspObj = controller.UpdateExistingInspection(inspection, action);
        return inspObj;
    }
    ...
    /// </summary>
    /// <param name="inspectionId"></param>
    [WebMethod(Description="Called by QA Website in order to delete the inspection and rollback track time records.")]
    public void DeleteInspection(string inspectionId)
    {
        IQAPBSController controller = QAPBSControllerFactory.GetQAPBSController(strcfSYS_NAME);
        controller.DeleteExistingInspection(inspectionId);
    }

    [WebMethod(Description = "Called by QA Website in order to get current status information for inspection prior to performing any action.")]
    public InspectionStatus GetInspectionStatus(string inspectionId, string userLogin)
    {
        IQAController controller = QAControllerFactory.GetQAController();
        return controller.GetInspectionStatus(inspectionId, userLogin);
    }


    //[WebMethod(Description="Test harness...")]
    public void TestHarness()
    {

        //ScheduledInspectionWrapper wrapper 
        //    = GetScheduledInspectionsForUser("amaldonado@sainspections.com", CompanyType.Inspector);

        //Create New Inspection Record  
        //string projectId = "000000000000000B";
        //string phaseName = "11";
        //string inspectionTypeId = "0000000000000001";
        //string inspectionScope = "Building";
        //string inspectedById = "000000000000053C";
        //string createdById = "000000000000053C";
        
        ////Inspected scope items
        //string[] arrScopeItems = new string[] { "B1", "B12" };
        //string newInspectionId = CreateNewInspection(projectId, phaseName, inspectionTypeId, inspectionScope, createdById, inspectedById, arrScopeItems);
        //string newInspectionId = "0000000000000054";

        //#region Load Inspection Test
        ////Now call load with Edit flag
        //Inspection insp = LoadInspectionById(newInspectionId, LoadActionsForInspection.OpenReadOnly);
        //#endregion

        //#region Documents Test (Update/Add/Delete)
        //Update inspection object and set the following:
        // New web links (Phase 2)        
        // New qa documents
        //List<QADocument> docList = new List<QADocument>();
        //QADocument doc = new QADocument();
        //doc.documentDesc = "Test Document";
        //doc.documentPath = "//test/filepath/1133444/word.doc";
        //doc.inspectionId = insp.inspectionId;
        //docList.Add(doc);
        
        //insp.qaDocs = docList.ToArray();
        // new custom inspection item

        // update existing inspection items : acknowledgement
        //List<QADocument> docList = new List<QADocument>();
        //foreach (QADocument doc in insp.qaDocs)
        //{
        //    doc.documentDesc = "updated test";
        //    doc.documentPath = "//testdirectory/testfile/mydocument.doc";
        //    doc.deleteDocument = true;
        //    docList.Add(doc);
        ...
        //#region Custom Inspection Item Test (Add/Update)
        //List<InspectionItem> inspItemList = new List<InspectionItem>();
        //InspectionItem inspItem = new InspectionItem();
        //inspItem.inspectionId = insp.inspectionId;
        //inspItem.itemDescription = "Custom Inspection Item Test";
        //inspItem.itemOrdinal = 99;
        //inspItem.categoryId = insp.miscCategory.categoryId;
        //inspItem.acknowledgeStatus = "Checked";
        //inspItemList.Add(inspItem);
        //insp.inspectionItems = inspItemList.ToArray();

        // update existing inspection items : acknowledgement
        //List<InspectionItem> docList = new List<InspectionItem>();
        //foreach (InspectionItem doc in insp.inspectionItems)
        //{
        //    //Update all to acknowledged
        //    doc.acknowledgeStatus = "Checked";
        //    docList.Add(doc);

        //}
        //insp.inspectionItems = null;
        //insp.inspectionItems = docList.ToArray();
        ...
        //updatedInspection = UpdateInspection(insp, ActionForInspection.Save);
        //#endregion

        //#region Delete

        ////DeleteInspection(newInspectionId);

        //#endregion

    }

    //[WebMethod(Description="Test Harness for notifications and workflow status transitions")]
    public void TestHarnessForWorkflowAndNotifications()
    { 
