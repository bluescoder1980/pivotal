    ...
    public class Product : IRFormScript
    {
        ...
        public virtual void BatchUpdateLotStatus()
        {
            try
            {
                ...
                ersiDataAccess objLib = (DataAccess)RSysSystem.ServerScripts[AppServerRuleData.DataAccessAppServerRuleName].CreateInstance();
                Opportunity ObjOpportunity = (Opportunity)RSysSystem.ServerScripts[modProduct.strsOPPORTUNITY].CreateInstance();
                ...
                rstLot = objLib.GetRecordset(modProduct.strqALL_ACTIVE_LOTS_NEWER, 1, dtQueryDate, modProduct.strfSALES_DATE, modProduct.strfRELEASE_DATE, modProduct.strfCONTRACT_CLOSE_DATE, modProduct.strfRESERVED_DATE, modProduct.strfLOT_STATUS, modProduct.strfINACTIVE, "Rn_Edit_Date",modProduct.strfPRODUCT_ID);
                ...
                for (lngLoop = 0; lngLoop <= lngRecordCount - 1; lngLoop += 1)
                {
                    ...
                    strNewStatus = GetLotStatusByRules(vntSoldDate, vntReleasedDate, vntClosedDate, vntReservedDate, TypeConvert.ToBoolean(vntInactive));
                    object vntLotId = rstLot.Fields[modProduct.strfPRODUCT_ID].Value;

                    if (strNewStatus == "Available" && strOldStatus == "Not Released")
                    {
                        //CMigles - Sept 30, 2010: Write Available history record when a lot becomes available. 
                        ObjOpportunity.WriteContractHistoryRecords(vntLotId, null, strNewStatus, DateTime.Today, false, null, false, true);
                    }
