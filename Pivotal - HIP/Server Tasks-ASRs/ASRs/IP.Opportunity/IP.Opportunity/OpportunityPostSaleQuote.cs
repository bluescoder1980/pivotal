    ...
    public class OpportunityPostSaleQuote : IRFormScript
    {
        ...
        protected virtual void ApplyPostSaleQuote(object postSaleQuoteOpportunityId)
        {
            try
            {
                ...
                //AM2011.02.14 - Commented out to ensure that option selections do not
                //interfere with creating dupliacte adjustments.  Irvine uses Adjustments
                //different that the way Homebuilder uses it out of the box.
                ...
                SendIPChangeOrderEmailNotification(postSaleQuoteOpportunityId, rstChangeOrder.Fields[modOpportunity.strfCHANGE_ORDER_ID].Value);
        ...
        /// 3.8.0.0        5/12/2006      DYin      Converted to .Net C# code.
        /// </history>
        public virtual void InactivateOtherPostSaleQuotes(object contractId, bool forAllPostSaleQuote)
        {
            try
            {
                DataAccess objLib = (DataAccess)RSysSystem.ServerScripts[AppServerRuleData.DataAccessAppServerRuleName].CreateInstance();
        ...
        /// 3.8.0.0        5/12/2006      DYin      Converted to .Net C# code.
        /// </history>
        protected virtual bool SendIPChangeOrderEmailNotification(object opportunityId, object changeOrderId)
        {
            try
            {
                DataAccess objLib = (DataAccess)RSysSystem.ServerScripts[AppServerRuleData.DataAccessAppServerRuleName].CreateInstance();
                ...
                //Get the Opportunity Product associated with the new Change Order and the create user of the change order
                object vntCreateUser = objLib.SqlIndex(modOpportunity.strtCHANGE_ORDER, modOpportunity.strfRN_CREATE_USER, changeOrderId);

                object vntCreateEmployee = objLib.SqlFind(modOpportunity.strtEMPLOYEE, modOpportunity.strfRN_EMPLOYEE_USER_ID, vntCreateUser);
                ...
                object vntOrgOpp_Id = RSysSystem.Tables[modOpportunity.strt_OPPORTUNITY].Fields[modOpportunity.strfPOST_SALE_ID].Index(opportunityId);
                strMessage = strMessage + TypeConvert.ToString(lngNBHD_Notification_Team.GetTextSub(modOpportunity.strdCHANGE_ORDER_CREATION_MESSAGE2,
                    new object[] { vntJob_Number, TypeConvert.ToString(TypeConvert.ToDateTime(RSysSystem.Tables[modOpportunity.strt_OPPORTUNITY].Fields[modOpportunity.strfECOE_DATE].Index(vntOrgOpp_Id)).ToShortDateString()) }));
                // "" & rstOpportunity.Fields(strfECOE_DATE).Value))

                
                //AM2010.11.17 - Added code to show the option the change order was created for
                StringBuilder sqlText = new StringBuilder();
                sqlText.Append("select op.product_name from change_order co ");
                sqlText.Append("inner join change_order_options coo on co.change_order_id = coo.change_order_id ");
                sqlText.Append("inner join opportunity__product op on coo.opportunity_product_id = op.opportunity__product_id ");
                sqlText.Append("where co.change_order_id = " + RSysSystem.IdToString(changeOrderId));
                
                
                Recordset rstChgOrderOpp = objLib.GetRecordset(sqlText.ToString());
                string strChgOrderFor = "Change Order created for : \n";

                if (rstChgOrderOpp.RecordCount > 0)
                {
                    rstChgOrderOpp.MoveFirst();

                    while (!rstChgOrderOpp.EOF)
                    {

                        strChgOrderFor = strChgOrderFor + TypeConvert.ToString(rstChgOrderOpp.Fields[0].Value) + "\n";
                        rstChgOrderOpp.MoveNext();
                    }
                    strMessage = strMessage + "\n\n" + strChgOrderFor;
                }
                rstChgOrderOpp.Close();



                strMessage = strMessage + TypeConvert.ToString(lngNBHD_Notification_Team.GetTextSub(modOpportunity.strdCHANGE_ORDER_CREATION_MESSAGE3,
                    new object[] { TypeConvert.ToString(objLib.SqlIndex(modOpportunity.strt_EMPLOYEE, modOpportunity.strfFIRST_NAME, vntSalesRepId)),
                         TypeConvert.ToString(objLib.SqlIndex(modOpportunity.strt_EMPLOYEE, modOpportunity.strfLAST_NAME, vntSalesRepId)), 
                        TypeConvert.ToString(objLib.SqlIndex(modOpportunity.strt_EMPLOYEE, modOpportunity.strf_WORK_EMAIL, vntSalesRepId)),
                        TypeConvert.ToString(objLib.SqlIndex(modOpportunity.strt_EMPLOYEE, modOpportunity.strfWORK_PHONE, vntSalesRepId))}));
                if (strEmailTo.Length > 0 && strSubject.Length > 0 && strMessage.Length > 0)
                {
