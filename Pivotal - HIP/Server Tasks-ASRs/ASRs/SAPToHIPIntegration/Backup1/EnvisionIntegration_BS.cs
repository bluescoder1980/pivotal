...
namespace CdcSoftware.Pivotal.Applications.HomeBuilders.EF.Server
{
    ...
    public partial class EnvisionIntegration : IRAppScript
    {
        ...
        protected virtual void GetPostSaleQuote(object contractId, out object postSaleQuoteId)
        {

            Recordset opportunityRecordset;

            //trying to find the in progress PSQ for the contract
            //2007-12-30 AB Changed PSQ query to also look for Post Build Quotes
            ...
            Recordset selectedOptionRecordSet = this.PivotalDataAccess.GetRecordset(OpportunityProductData.OpportunityProductForOpportunityDivProdAndProdNameQueryName
                ...
                if (selectedOptionRecordSet.RecordCount == 0)
                {
                    ...
                    else
                    {
                        ...
                        AddOptionToPostSaleQuote(postSaleQuoteId, nbhdp_ProductId, transaction.Quantity, transaction.DateCreated, transaction.Price
                            , out oppProductId, out oppProductLocId);

                        //AAB 2010-05-17 Added to handle new option mappings
                        SetNotes(transaction, oppProductId);

                        //set the option selection notes and set Option Selection Source as Envision.
                        /*EditTableRecords(oppProductLocId, OppProductLocationData.TableName
                            , OppProductLocationData.NotesField, GetNotes(transaction)
                            , OppProductLocationData.OptionSelectionSourceField, OptionSelectionSource.Envision
                            );*/
                        //AAB 2010-05-17 This was customized to support one to one envision mapping
                        EditTableRecords(oppProductLocId, OppProductLocationData.TableName
                            , OppProductLocationData.OptionSelectionSourceField, OptionSelectionSource.Envision
                            );
            ...
            )
        {
            ...
            //06-11-2008 AB changed to support selecting codes based on non-Pivotal Id key
            ...
            //if any value is null use the default
        ...
        ///     Add non-custom option to the post sale quote
        ///     optionNumber is Division_Product_Id
        ///     This was created because Pivotal must accept the price for options that are added in Envision
        ///     The customer is quoted the Envision price.
        /// </summary>
        /// <param name="postSaleQuoteId">postSaleQuoteId</param>
        /// <param name="nbhdp_ProductId">nbhdp_ProductId</param>
        /// <param name="quantity">quantity</param>
        ...
        protected virtual void AddOptionToPostSaleQuote(object postSaleQuoteId, object nbhdp_ProductId, int quantity, DateTime transactionDatetime
            ,decimal price, out object OppProductId, out object OppProductLocId)
        {
            ...
            transitParams.SetUserDefinedParameter(5, OptionSelectionSource.Envision);
            transitParams.SetUserDefinedParameter(6, transactionDatetime);
            //transitParams.SetUserDefinedParameter(7, price);
            transitParams.SetUserDefinedParameter(7, price / quantity);

            object parameterList = transitParams.ParameterList;

            m_rdaSystem.Forms[FormName.HBOpportunityOptions].Execute("CreateEnvisionOpportunityProductOption", ref parameterList);
        ...
        protected virtual byte[] GetMostSpecificNbhdpProductFromContractAndDivisionProduct(object ContractId, object divisionProductId)
        {
            ...
            //AB 2008-04-13 Update existing query take elevation into account
            ...
            nbhdpProductRecordset = this.PivotalDataAccess.GetRecordset("Env: MI Options Available For Plan ? And DivProd ?"
                        , 9, (object)divisionProductId, planId, planCode, planCode, elevationCode, regionId, divisionId, neighborhoodId, releaseId
                        , NBHDPProductData.NBHDPProductIdField, NBHDPProductData.WCLevelField
                                                        );
        ...
        protected virtual void SetNotes(TransactionType transaction, object oppProdId)
        {
            ...
            if (transaction.Notes != null)
            {
                ...
                EditTableRecords(oppProdId, "Opportunity__Product", "MI_Color_Notes", colorNotes, "MI_Style_Notes", styleNotes
                                    , "CustomerInstructions", locationNotes, "OptionNotes", otherNotes); 
            }
