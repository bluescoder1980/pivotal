    ...
    public class Option : IRFormScript
    {
        ...
        public void DoOptionLookupLogic(Recordset rstPrimary, bool blnIsNewRecord)
        {
            try
            {
                ...
                {
                    // Neighborhood_Id was successfully looked-up, so write it to NBHDP_Product.Neighborhood_Id & continue...
                    ...
                    if (!(String.IsNullOrEmpty(strExtSrcPhaseCode)))
                    {
                        ...
                        {
                            strErrMsg = "Option Rejected - No NBHD_Phase (Release Phase) record found for the supplied Neighborhood & Phase lookup values";
                            throw new PivotalApplicationException(strErrMsg);
                        }
                        else
                        {
                    ...
                    {
                        strErrMsg = "Option Rejected - No Plan record found for the supplied Neighborhood & Plan lookup values";
                        throw new PivotalApplicationException(strErrMsg);
                    }
                    else
                    {
