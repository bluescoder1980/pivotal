    ...
    public class Lot : IRFormScript
    {
        ...
        protected virtual bool CheckRequiredFields(Recordset rstPrimary, ref string strErrorMessage)
        {
            try
            {
                ...
                {
                    strErrorMessage = "Lot Rejected - Neighborhood must be supplied";
                    return false;
                }
        ...
        protected virtual void DoLotLookupLogic(Recordset rstPrimary, bool blnIsNewRecord)
        {
            try
            {
                ...
                {
                    strErrMsg = "Lot Rejected - No Neighborhood record found for the supplied Neighborhood lookup value";
                    throw new PivotalApplicationException(strErrMsg);
                }
                else
                {
                    // if Product.Neighborhood_Id is null, and it should be on all new Product records, then 
                    ...
                    {
                        // if Neighborhood_Id is not null, and SAP has sent a Neighborhood_Id which differs from what
                        ...
                        {
                            strErrMsg = "Lot Rejected - SAP is attempting to change the Neighborhood on this Lot record, which is not allowed";
                            throw new PivotalApplicationException(strErrMsg);
                        }
                    }
                }
                ...
                rstPrimary.Fields[IntegrationConstants.strfEXTERNAL_SOURCE_NAME].Value = "SAP";
