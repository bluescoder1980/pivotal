        ...
        public object UpdateInventoryQuote(IRSystem7 rSys, object vntQuoteId, Recordset rstPrimary, object vntActiveContractOrReservationId)
        {
            try
            {
                ...
                if ((vntActiveContractOrReservationId != null) && (!(Convert.IsDBNull(vntActiveContractOrReservationId))))                    
                {
                    ...
                    {
                        // If they differ, then don't update the IQ.Additional_Price field with the supplied Base Price
                        // Instead, update the TIC_Future_Change_Price field *on the active Contract or Reservation* with the supplied Base Price
                        this.UpdateActiveContractOrReservationFutureChangePrice(rSys, 
                                      ...
                                      bool blnUpdateDescription)
        {
            try
            {
                ...
                if (rst.RecordCount > 0)
                {
                    //Found it
                    rst.MoveFirst();
                    vntSubCategoryId = rst.Fields[0].Value;
                }
                else
                {
