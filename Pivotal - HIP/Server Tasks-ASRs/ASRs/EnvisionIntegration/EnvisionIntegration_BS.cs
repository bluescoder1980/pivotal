...
namespace CdcSoftware.Pivotal.Applications.HomeBuilders.EF.Server
{
    ...
    public partial class EnvisionIntegration : IRAppScript
    {
                                ...
                                if (validateBuyerSelectionReturn.Length == 0)
                                {
                                    switch (transaction.TransactionType1)
                                    {
                                        ...
                                        case TransactionTypeTransactionType.Edit:
                                            AddEditOptionToPostSaleQuote(postSaleQuoteId, divisionProductId, productName, productNumber, nbhdp_ProductId, optionAvailableTo, transaction, false, null);
                                            break;
                                        case TransactionTypeTransactionType.Delete:
                                            DeleteOptionOnPostSaleQuote(postSaleQuoteId, divisionProductId, productName, productNumber, transaction, false);
                                            break;
                                        default:
        ...
        protected virtual void AddEditOptionToPostSaleQuote(object postSaleQuoteId, object divisionProductId, string productName, string productNumber, object nbhdp_ProductId, string optionAvailableTo, TransactionType transaction,
            bool isPreplot, object contractId)
        {

            //AM2011.03.03 - Added isPreplot to query parameter to distinguish between pre-plot and non-preplot options
            Recordset selectedOptionRecordSet = this.PivotalDataAccess.GetRecordset(OpportunityProductData.OpportunityProductForOpportunityDivProdAndProdNameQueryName
                    , 5, postSaleQuoteId, divisionProductId, productName, productNumber, isPreplot
            ...
            )
        {
            ...
            //AM2011.01.24 - no need to set in code, since this is a formula.  Setting divisionproduct will take care of this.
            ...
            //AM2011.02.11 - Set to D for all options created from Chateau
            opportunityProductRecordset.Fields[TICEnvisionConstants.strcTIC_SHOW_IN_DOCUMENT].Value = "D";
            ...
            //object divProdId = util.GetDivisionCustomOption(TICEnvisionConstants.strcCUSTOM_OPTION, m_rdaSystem);
            //if (divProdId == null)
            //{
            //    throw new PivotalApplicationException("Division Product must be configured for 'Custom Option' at the division level.");
            //}
            //else
            //{
            //    //opportunityProductRecordset.Fields[OpportunityProductData.DivisionProductIdField].Value = divProdId;
            //}
            ...
            if (categoryNumber != null) // && catId != null)
            {
                ...
                catId = util.FindCategory(m_rdaSystem, categoryNumber, string.Empty, string.Empty, true);
                //AM2011.01.24 - Category was not getting set in this block
            ...
            ,decimal price, out object OppProductId, out object OppProductLocId, bool isPreplot)
        {
            ...
            Recordset rstOppProd = this.PivotalDataAccess.GetRecordset(OppProductId, OpportunityProductData.TableName,
                ...
                TICEnvisionConstants.strfTIC_PREPLOT_OPTION, TICEnvisionConstants.strcTIC_SHOW_IN_DOCUMENT);
            //AM2010.09.08 - Set all Chateau options to built
            rstOppProd.Fields[OpportunityProductData.BuiltOptionField].Value = 1;

            //AM2011.02.11 - Added new attribute to allow custom options and design center options to be flagged as "D" for decorator.
            rstOppProd.Fields[TICEnvisionConstants.strcTIC_SHOW_IN_DOCUMENT].Value = "D";
            ...
            //This check compares the price defined at the Option Configuration to the price on the option
            //selection.  If the two are different it flags it as a Price mismatch.
        ...
        protected virtual byte[] GetMostSpecificNbhdpProductFromContractAndDivisionProduct(object ContractId, object divisionProductId)
        {
            ...
            object planId, releaseId, neighborhoodId, divisionId, regionId, lotId, constructionProjectId;
            string planCode;
            ...
            //AM2010.11.03 - Get the Lot record on the contract in order to get the Construction Project
            lotId = m_rdaSystem.Tables[OpportunityData.TableName].Fields[OpportunityData.LotIdField].Index(ContractId);
            constructionProjectId = m_rdaSystem.Tables[ProductData.TableName].Fields["TIC_Construction_Project_Id"].Index(lotId);

            if (Convert.IsDBNull(constructionProjectId) || constructionProjectId == null)
            {
                string errorMessage = "Lot does not have a constrution project.  Must have a construction project associated to integrate option selections";
                throw new PivotalApplicationException(errorMessage);
            }

            //AM2010.11.04 - Commented out to use the Construction Project query
            ...
            nbhdpProductRecordset = this.PivotalDataAccess.GetRecordset("Env: Options Available For Plan ? And DivProd ? - TIC"
                        , 7, (object)divisionProductId, planId, planCode, regionId, divisionId, constructionProjectId, releaseId
                        , NBHDPProductData.NBHDPProductIdField, NBHDPProductData.WCLevelField);
        ...
        protected virtual void EditOppProdLocAndOppProd(object oppProductId, TransactionType transaction)
        {
            ...
            if (transaction.Option.OptionType == SelectedOptionTypeOptionType.Custom)
            {
                ...
                bool blnPriceMismatch = false;
               

                //AM2011.02.15 - added logic to handle price mismatches
                if (cusPrice != transaction.Price)
                {
                    blnPriceMismatch = true;                    
                }
                //Always set price to price coming in
                cusPrice = transaction.Price;
                ...
                EditTableRecords(oppProductId, OpportunityProductData.TableName
                    ...
                    , TICEnvisionConstants.OPPORTUNITY__PRODUCT_PRICE_MISMATCH, blnPriceMismatch
        ...
        private string ProcessOptionSelection(Recordset rstOptionSelection)
        {
            ...
            try
            {
                ...
                {                    
                    ...
                    object neighborhoodId = null;
                    object vntConstrProjectId = util.FindConstructionProject(m_rdaSystem,
                        TypeConvert.ToString(rstOptionSelection.Fields[TICEnvisionConstants.TICIntOptionSelectionsTable.TableFields.NEIGHBORHOOD].Value), ref neighborhoodId);
                ...
                else
                {
                    ...
                    if (!Convert.IsDBNull(contractId))
                    {
                        ...
                        if (validateBuyerSelectionReturn.Length == 0)
                        {
                            ...
                            if (intQuantity > 0)
                            {
                                AddEditOptionToPostSaleQuote(vntPostSaleQuoteId, divisionProductId, productName, productNumber, nbhdProductId, optionAvailableTo, transType, isPrePlot, contractId);
                            }
                            else
                            {
                                DeleteOptionOnPostSaleQuote(vntPostSaleQuoteId, divisionProductId, productName, productNumber, transType, isPrePlot);
                            }
                        }
                        else
                        {
        ...
        private object GetNeighborhoodProduct(string neighborhoodCode, string optionNumber, out object divisionProductId)
        {
            ...
            //Comment out Neighborhood query 
            //Get Customer SQL
            ...
            //Get NBHDP_Product by ConstructionProject
            sqlText.Append("SELECT NBHDP_PRODUCT_ID, DIVISION_PRODUCT_ID FROM ");
            sqlText.Append("NBHDP_PRODUCT NP INNER JOIN TIC_CONSTRUCTION_PROJECT TC ");
            sqlText.Append("ON NP.TIC_CONSTRUCTION_PROJECT_ID = TC.TIC_CONSTRUCTION_PROJECT_ID ");
            sqlText.Append("WHERE TC.EXTERNAL_SOURCE_COMMUNITY_ID ='" + neighborhoodCode + "'");
            sqlText.Append("AND NP.EXTERNAL_SOURCE_ID = '" + optionNumber + "'");
