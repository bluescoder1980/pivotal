...
{
    public class Escrow : IRFormScript
    {
        ...
        public virtual void Execute(IRForm pForm, string MethodName, ref object ParameterList)
        {
            try
            {
                ...
                switch (MethodName)
                {
                    ...
                    default:
                        string message = MethodName + TypeConvert.ToString(RldtLangDict.GetText(modEscrow.strdINVALID_METHOD));
                        parameterArray = new object[] { message };
                        throw new PivotalApplicationException(message, modEscrow.glngERR_METHOD_NOT_DEFINED);
                }
